...
int logFile(std::set<uint64_t> missingTxs, CNode* pfrom,std::string header,std::string fileName)
{
    ...
    std::vector<CTransactionRef> txid;
    ...
    if(missingTxs.size() > 0)
        fnOut << timeString << ("GRMISSINGTX - Missing %d transactions from graphene block: %s",missingTxs.size()) << std::endl;
    else
        logFile(("GRNOMISSINGTXS - All txs needed to reconstruct this graphene block were found in our mempool: " + header ));
...
void dumpMemPool(std::string fileName, INVTYPE type, INVEVENT event, int counter)
{
    ...
    sysCmd = "bitcoin-cli -datadir=" + GetDataDir().string() + " getmempoolinfo > " + mempoolFile;
    (void)system(sysCmd.c_str());
    sysCmd = "bitcoin-cli -datadir=" + GetDataDir().string() + " getrawmempool >> " + mempoolFile;
    (void)system(sysCmd.c_str());
